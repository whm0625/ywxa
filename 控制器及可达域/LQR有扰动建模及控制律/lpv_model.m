...
syms V miu alpha q x h T deltae Vs mius alphas qs xs hs Ts deltaes Vr miur alphar qr xr hr Tr deltaer t dV dmiu dalpha dq dx dh dT ddeltae Vw;
m=0.8;      rho=1.225;
c=0.25;     b=1;
...
% D=1/2*rho*V^2*S*CD;
% M=1/2*rho*V^2*S*CM;
L=1/2*rho*(V+Vw)^2*S*CL;
D=1/2*rho*(V+Vw)^2*S*CD;
M=1/2*rho*(V+Vw)^2*S*CM;
...
% Vwr = -0.5-sin(1.43*pi*t);

Vw1 = [-0.5-sin(1.43*pi*tz);-0.5;];
if tz<=0.7
    Vwr=subs(Vw1(1,1),tz,t);
else
    Vwr=subs(Vw1(2,1),tz,t); 
end
...
Soriginxin=subs(Soriginxin,Vw,Vwr);
